...
@pytest.mark.asyncio
async def test_crypto_exit_worker_defaults_to_hard_max_hold_when_flag_missing(monkeypatch):
    worker = CryptoExitWorker()
    db = AsyncMock()
    position = SimpleNamespace(
        id=uuid4(),
    ...
    monkeypatch.setattr("app.crypto.exit_worker.kraken_client.get_ticker", AsyncMock(return_value={"c": ["105.0"]}))
    monkeypatch.setattr("app.crypto.exit_worker.kraken_client.get_ohlcv", AsyncMock(side_effect=AssertionError("ohlcv should not be fetched after fallback hard max hold exit")))
