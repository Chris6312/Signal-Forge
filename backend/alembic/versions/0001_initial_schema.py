...
def upgrade() -> None:
    bind = op.get_bind()
    if sa.inspect(bind).has_table("watchlist_symbols"):
        # Tables were pre-created outside Alembic (e.g. SQLAlchemy create_all).
        # Skip DDL so subsequent migrations can run and stamp alembic_version.
        return
